...
export default function DailyTasksPopout({ isOpen, onClose }: DailyTasksPopoutProps) {
  const { dailyItems, updateDailyItem } = usePotion();

  const completedDailyItems = dailyItems.filter(item => item.completed).length;
  const totalHours = dailyItems.reduce((sum, item) => sum + item.hours, 0);
        ...
        {isOpen && (
          <>
            ...
            <div className="sticky top-0 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 z-10">
              <div className="p-4">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Daily</h2>
              ...
              <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border-t border-gray-200 dark:border-gray-700">
                <div className="flex items-center gap-2">
                  <div className="px-2 py-1 bg-blue-100 dark:bg-blue-800/50 text-blue-700 dark:text-blue-300 rounded-md text-xs font-medium">
                    {completedDailyItems}/{dailyItems.length} completed
                  </div>
                  <div className="px-2 py-1 bg-blue-100 dark:bg-blue-800/50 text-blue-700 dark:text-blue-300 rounded-md text-xs font-medium">
                    {totalHours}h total
                  </div>
                  {completedDailyItems === dailyItems.length && dailyItems.length > 0 && (
                ...
                {dailyItems.length === 0 ? (
                  ...
                  dailyItems.map((item) => (
                    ...
                    >
                      ...
                      {item.hours > 0 && (
                        <div className={`px-1.5 py-0.5 rounded-full text-xs font-medium ${getHoursColor(item.hours)}`}>
                          {item.hours}h
                        </div>
                      )}
                    </div>
